...
with DAG(
    ...
    data_ingestion = PythonOperator(
        ...
        python_callable=lambda: ingest_data()
    )

    data_drift = PythonOperator(
        task_id='data_drift',
        python_callable=lambda: main()
    )

    data_preprocessing = PythonOperator(
        task_id='data_preprocessing',
        python_callable=lambda: preprocess_data()
    )

    data_profiling = PythonOperator(
        task_id='data_profiling',
        python_callable=lambda: profile_data()
    )

    hyperparameter_optimization = PythonOperator(
        task_id='hyperparameter_optimization',
        python_callable=lambda: perform_hyperparameter_optimization()
    )

    model_training = PythonOperator(
        task_id='model_training',
        python_callable=lambda: train_best_model(
            load_best_params()['best_model_name'], 
            load_best_params()['best_params']
        )
    )

    download_model_task = PythonOperator(
        task_id='download_model',
        python_callable=lambda: download_model('registerd_model')
    )

    json_reading = PythonOperator(
        task_id='json_reading',
        python_callable=lambda: print_file_contents('best_model_and_params.json')
    )
    ...
    success_email = PythonOperator(
        ...
        trigger_rule='all_success'  # Send success email only if all tasks succeed
    )
    ...
    # BashOperator Task to build and push Docker image
    run_docker_script = BashOperator(
        task_id='run_bash_docker_script',
        bash_command=(
            "cd /home/sigmoid/Documents/airflow_mlops && "
            "docker build -t nileshkishore2001/airflow_iris_model:{{ run_id }} . && "
            "docker push nileshkishore2001/airflow_iris_model:{{ run_id }}"
        )
    )
    
    # BashOperator Task to deploy on Kubernetes
    deploy_on_k8s = BashOperator(
        task_id='deploy_on_k8s',
        bash_command=(
            "cd /home/sigmoid/Documents/airflow_mlops && "
            "sed -e 's|<DOCKER_IMAGE_TAGGED>|nileshkishore2001/airflow_iris_model:{{ run_id }}|' "
            "k8s/combined-deployment-and-service.yaml > k8s/deployment-final.yaml"
        )
    )
    ...
    # Failure email dependency only on `one_failed`
